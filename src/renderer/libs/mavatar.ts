...
export async function mavatar(
  ...
  const themes: any = {
    ...
    '05': {
      ...
      B: {
        ...
        top: [color.good_black, color.good_black],
      },
      C: {
        env: ['#884f00'],
        clo: [
  ...
  const envSquare =
    '<rect cx="115.5" cy="115.5" width="230" height="230" style="fill:#01;"/>';
  const env = shape == 'square' ? envSquare : envCircle;
  ...
  const sha256Hash = await sha256(seedNum);
  console.log(sha256Hash, seedNum);
  const sha256Numbers = sha256Hash.toString().replace(/\D/g, '');

  const hash = sha256Numbers.substring(0, 12);
  ...
  const final: any = [];
  function getFinal(part: any, partV: any, theme: any) {
    // console.log(part, partV, theme)
    const colors = themes[partV][theme][part];
    const svgString = sP[partV][part];
    // console.log(colors, svgString);

    const regex = /#(.*?);/g;
    const result = svgString.match(regex);

    let resultFinal = svgString;
  ...
  for (const part in p) {
    const partV = p[part].substring(0, 2);
    const theme = p[part].substring(2, 3);
    // // console.log(part, partV, theme);
