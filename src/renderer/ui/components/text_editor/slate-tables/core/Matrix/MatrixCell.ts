...
export class MatrixCell {
  public readonly node: NodeEntry<TableCellElement>[0];

  public readonly path: NodeEntry<TableCellElement>[1];
  ...
  get nextCell(): MatrixCell | undefined {
    return (
      this.row.cells.at(this.x + 1) ??
      this.matrix.columns.at(0)?.cells.at(this.y + 1)
    );
  }
